...
def test_pronoun_forwarding_defaults_on_and_is_overridable_per_connector_kind(
    tmp_path, isolated_env, monkeypatch
):
    monkeypatch.setenv("STOAT__0__PRONOUN_FORWARDING", "false")
    path = _write_config(
        tmp_path,
        """
        discord:
    ...
    config = load_config(path)
    assert config.discord[0].pronoun_forwarding is False  # literal override
    assert config.stoat[0].pronoun_forwarding is False  # env override
